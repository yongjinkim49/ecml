...
class LossHistory(keras.callbacks.Callback):
	...
	def on_epoch_end(self, epoch, logs={}):
		...
		print("Training takes {:.1f} secs. Test losses: {}".format(
...
class KerasWorker(object):
	...
	def compute(self, config, budget, working_directory, history, *args, **kwargs):
		...
		return ({
			...
			'cur_loss': 1-val_score[1],
			'cur_acc' : test_score[1],
			'cur_iter' : int(budget),
			'iter_unit': 'epoch',
...
if __name__ == "__main__":
	...
	print(config)
	res = worker.compute(config=config, budget=9, working_directory='.', history=history)
	print(res)
